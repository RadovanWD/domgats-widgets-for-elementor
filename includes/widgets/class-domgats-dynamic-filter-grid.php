...
=======
	...
	protected function render() {
		...
		$script_data = function_exists('dgwfe_get_script_data') ? dgwfe_get_script_data() : [];

		$config = [
			'widgetId'        => $widget_id,
			'restUrl'         => $script_data['restUrl'] ?? esc_url_raw(rest_url(Rest_Controller::REST_NAMESPACE . '/grid')),
			'ajaxUrl'         => $script_data['ajaxUrl'] ?? esc_url_raw(admin_url('admin-ajax.php')),
			'nonce'           => $script_data['nonce'] ?? wp_create_nonce(DGWFE_NONCE_ACTION),
			'nonceAction'     => $script_data['nonceAction'] ?? DGWFE_NONCE_ACTION,
			'transport'       => $script_data['transport'] ?? 'rest',
			'paginationType'  => $settings['pagination_type'],
			'layout'          => $settings['layout_type'],
			'animation'       => $settings['animation_style'],
			'stagger'         => isset($settings['animation_stagger']) ? (int) $settings['animation_stagger'] : 80,
	...
	private function render_grid(array $settings, array $items, $animation_class, $wrap = true) {
		...
		foreach ($items as $index => $item) {
			$delay = isset($settings['animation_stagger']) ? ((int) $settings['animation_stagger'] * $index) : 0;
			$style_attr = $delay ? ' style="--domgats-delay:' . esc_attr($delay) . 'ms;"' : '';
			echo '<article class="domgats-card ' . esc_attr($animation_class) . '"' . $style_attr . ' data-animate-delay="' . esc_attr($delay) . '" role="listitem">';
			$this->render_card_contents($settings, $item);
			echo '</article>';
		}
